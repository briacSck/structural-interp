...
counterfactual failure (Spearman 0.87 for out-of-distribution transport and
0.80 for the subsidy experiment; permutation $p = 0.0001$ and $0.0007$).
Internal audits, we argue, are usefully understood as specification tests on
mechanisms, and their scope is delimited exactly by the invariance of the
cognitive primitive.
\end{abstract}
...
(regime transport; permutation $p = 0.0001$) and 0.80 (subsidy;
$p = 0.0007$), using in-distribution activations only.
...
\citet{bugniura2019} derive the bias from local misspecification; and
\citet{andrews2025} develop the general theory of structural estimation
under misspecification and its implications for practice. Our laboratory
realizes these results concretely --- the categorical agents are the
misspecification, the biased $\rhat$ is the predicted consequence --- and
adds the diagnostic layer. In machine-learning dialect the same problem
...
\citep{gabaix2014}; two \emph{RI-motivated attenuation} agents blending
toward the regime-blind policy with weight $\lambda \in \{0.25, 0.75\}$
\citep{matejkamckay2015, fosgerau2020};\footnote{A linear CCP blend, not
the multiplicative tilt $P(i) \propto P_0(i)\exp(v_i/\lambda)$ of the exact
Mat\v{e}jka--McKay RI-logit; we label it accordingly and designate the exact
form as a robustness exercise. Likewise the sparse-max agents act optimally
on the perceived state rather than re-solving the sparsified program ---
the standard implementation shortcut, flagged here.} and two
\emph{salience-asymmetric} agents that ignore regimes below (above) the
median. Identical architectures and inputs mean
...
data, no knowledge of the true parameters. Two properties of the design
deserve emphasis. First, \emph{independence}: the audit is computed from
weights and activations at states the agent actually visits, before any
counterfactual data exist --- the counterfactual outcomes it predicts are
held-out objects computed later from a different environment, so the audit
and its target are not the same quantity measured twice. Second,
\emph{inference}: at $n = 13$ agents, the headline rank correlation of
\textbf{0.87} has a one-sided Monte-Carlo permutation $p = 0.0001$,
bootstrap 95\% CI $[0.49, 0.97]$, and a leave-one-out range of
$[0.82, 0.92]$ --- no single agent drives it (single channels alone: 0.53
and 0.18). Within the categorical family, where coarseness varies cleanly,
the ordering happens to be perfect (Spearman 1.00, $n = 6$, exact one-sided
$p = 1/6! \approx 0.0014$); at that sample size we present it as an
illustration, not a headline. One caveat we flag rather than hide: audit
scores are normalized across the agent set (min--max), so the level of any
one score is composition-dependent; the leave-one-out band above bounds how
much this matters here, and an absolute calibration is the designated
refinement. The residual imperfection is honest and documented: the
...
(permutation $p = 0.0007$, bootstrap 95\% CI $[0.39, 0.92]$, leave-one-out
range $[0.75, 0.85]$; 0.63 for the rate error).
...
\bibitem[Andrews et al.(2025)]{andrews2025}
Andrews, I., Barahona, N., Gentzkow, M., Rambachan, A., \& Shapiro, J.~M.
(2025). Structural estimation under misspecification: Theory and
implications for practice. \emph{Quarterly Journal of Economics}, 140(3),
1801--1861.
